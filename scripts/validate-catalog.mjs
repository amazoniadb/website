...
for (const [index, record] of (catalog || []).entries()) {
  ...
  if (record.responsibleUse != null) {
    if (!record.responsibleUse || typeof record.responsibleUse !== "object" || Array.isArray(record.responsibleUse)) {
      issues.push(`${label}: responsibleUse must be an object when present.`);
    } else {
      const allowedKeys = new Set(["note", "protocolUrl", "communityAuthorityUrl"]);
      const unsupportedKeys = Object.keys(record.responsibleUse).filter((key) => !allowedKeys.has(key));
      if (unsupportedKeys.length) issues.push(`${label}: responsibleUse has unsupported field(s): ${unsupportedKeys.join(", ")}.`);
      const note = record.responsibleUse.note;
      if (!note || typeof note !== "object" || Array.isArray(note)) {
        issues.push(`${label}: responsibleUse.note must be a localized object.`);
      } else {
        const locales = ["en", "pt-BR", "es"];
        const unsupportedLocales = Object.keys(note).filter((locale) => !locales.includes(locale));
        if (unsupportedLocales.length) issues.push(`${label}: responsibleUse.note has unsupported locale(s): ${unsupportedLocales.join(", ")}.`);
        for (const locale of locales) {
          if (typeof note[locale] !== "string" || !note[locale].trim()) {
            issues.push(`${label}: responsibleUse.note is missing ${locale} copy.`);
          }
        }
      }
      for (const field of ["protocolUrl", "communityAuthorityUrl"]) {
        if (record.responsibleUse[field] != null && !isHttpUrl(record.responsibleUse[field])) {
          issues.push(`${label}: responsibleUse.${field} must be a valid http:// or https:// URL.`);
        }
      }
    }
  }
