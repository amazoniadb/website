...
const OPTIONAL_RECORD_KEYS = ["temporalCoverage", "spatialResolution", "license", "methodologyUrl", "tags"];
...
if (ghOutput) {
  const delimiter = `amazoniadb_${Date.now()}_${Math.random().toString(16).slice(2)}`;
  await appendFile(ghOutput, `title<<${delimiter}\n${record.title}\n${delimiter}\nslug=${record.id}\n`);
}
