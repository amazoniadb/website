(() => {
  ...
  const compactAccessLabels = {
    "Provider terms apply": "Términos del proveedor",
    "Dataset-specific license": "Licencia del conjunto",
    "Publicly available": "Acceso público"
  };
  const kindLabels = { "Dataset": "Conjunto de datos", "Data portal": "Portal de datos", "Download": "Descarga", "Explorer": "Explorador" };
  const formatLabels = {
    "Dashboard": "Panel",
    "Graphs/plots": "Gráficos",
    "Interactive visualizations": "Visualizaciones interactivas",
    "PDF reports": "Informes PDF",
    "R package": "Paquete R",
    "Web dashboard": "Panel web",
    "Web encyclopedia": "Enciclopedia web",
    "Web map": "Mapa web"
  };
  const detailLabels = { timeframe: "Período", resolution: "Resolución", license: "Licencia", coverage: "Cobertura", access: "Acceso", review: "Revisión del enlace", methodology: "Documentación", sourcePage: "Página de la fuente" };
  const compactLicenseLabels = {
    ...
    "HydroSHEDS License Agreement": "Licencia HydroSHEDS",
  ...
  const researchLabels = {
    ...
    addPath: "Añadir estas fuentes",
    pathAdded: "Fuentes de la ruta añadidas a la selección.",
    pathAlreadyPresent: "Las fuentes de esta ruta ya están en la selección.",
    pathPartiallyAdded: (count) => `Se ${count === 1 ? "añadió 1 fuente" : `añadieron ${count} fuentes`}; el límite de seis impidió añadir las demás.`,
    questionPosition: (current, total) => `Ruta ${current} de ${total}.`
  };
  const sourceSetLabels = {
    add: "Añadir",
    added: "En selección",
    remove: "Quitar",
    removed: (title) => `${title} se quitó de la selección.`,
    full: "Se alcanzó el límite de seis fuentes.",
    count: (sources, areas) => `${sources} ${sources === 1 ? "fuente" : "fuentes"} · ${areas} ${areas === 1 ? "área principal" : "áreas principales"}`,
    responsible: "Cruzar fuentes no demuestra impacto, consentimiento ni tenencia. Consulte los avisos de uso responsable y la documentación de cada fuente.",
    responsibleLabel: "Uso responsable",
    protocol: "Protocolo de la fuente",
    communityAuthority: "Autoridad comunitaria",
    formats: "Formatos",
    facts: "Datos de la fuente",
    moreTopics: (count) => `${count} ${count === 1 ? "tema adicional" : "temas adicionales"}`,
    copySuccess: "Enlace copiado",
    copyFailure: "No se pudo copiar",
    reviewed: "Enlace revisado el",
    emptyCountry: "Todavía no hay una fuente específica para este país. Prueba Panamazonía o Global — recortable.",
    emptyDefault: "Ninguna fuente coincide con esos filtros. Prueba una búsqueda más amplia."
  };
  ...
  const topicValues = populateControlledSelect(topic, topicVocabulary, topicLabels, "Todos los temas");
  const roleValues = populateControlledSelect(role, roleVocabulary, roleLabels, "Cualquier finalidad");
  const modeValues = populateControlledSelect(mode, modeVocabulary, modeLabels, "Cualquier tipo");
  const timeValues = populateControlledSelect(timeProfile, timeVocabulary, timeLabels, "Cualquier período");
  ...
  const updateMoreFilterSummary = () => {
    ...
    moreFilterCount.textContent = activeCount ? `· ${activeCount} ${activeCount === 1 ? "activo" : "activos"}` : "";
  ...
  const downloadSourceSet = () => {
    ...
    link.href = href;
    link.download = "amazoniadb-conjunto-de-fuentes.csv";
  ...
  const renderDomains = () => {
    if (!categories.length) {
      domainNav.innerHTML = '<p class="empty-state">Los filtros por área no están disponibles temporalmente. Aún puede explorar el catálogo completo.</p>';
      return;
    }
      ...
      <article class="dataset-card${state.source === record.id ? " is-discovery" : ""}" data-record-id="${escapeHtml(record.id)}" aria-labelledby="source-${escapeHtml(record.id)}-title"${state.source === record.id ? " tabindex=\"-1\"" : ""}>
        ...
        <div class="card-actions">
          ...
          <div class="card-tools">
            ...
            <button class="cite-button" type="button" data-report-id="${escapeHtml(record.id)}">Reportar problema</button>
          </div>
        </div>
      </article>`;
    }).join("");
  ...
  const renderDiscovery = () => {
    ...
    discoveryResult.textContent = `Mostrando ${record.title} — una fuente con enlace revisado en ${category?.label || record.category} · ${coverageLabels[record.coverage] || record.coverage}.`;
  };
  ...
  const injectStructuredData = () => {
    const structuredData = {
      ...
      "@graph": [
        ...
        {
          "@type": "DataCatalog",
          "@id": "https://amazoniadb.org/es/#catalog",
          "name": "AmazoniaDB",
          "description": "Un directorio ligero de conjuntos de datos socioambientales de la Amazonía, indexados en sus fuentes originales.",
          "url": "https://amazoniadb.org/es/",
          "inLanguage": "es-419",
  ...
  clearSourceSetButton?.addEventListener("click", () => {
    state.selectedIds = [];
    if (sourceSetStatus) sourceSetStatus.textContent = "Selección vaciada.";
    renderCatalog();
    renderSourceSet();
    syncUrl();
  ...
  grid.addEventListener("click", async (event) => {
    ...
    if (citeButton) {
      ...
      if (!record) return;
      const citation = `"${record.title}." ${record.provider}. Enlace revisado por AmazoniaDB el ${record.checked}. Consultado el ${formatDate(new Date().toISOString().slice(0, 10))}. ${record.url}`;
    ...
    if (reportButton) {
      ...
      if (!record) return;
      const subject = encodeURIComponent(`Problema con una fuente: ${record.title}`);
      const body = encodeURIComponent(`Entrada: ${record.id}\nURL: ${record.url}\n\nTipo de problema (no incluya ubicaciones ni datos sensibles en este mensaje):\n[ ] Enlace\n[ ] Contexto o metadatos\n[ ] Cuestión territorial o de derechos\n[ ] Información sensible\n[ ] Solicitud de protocolo, corrección o eliminación\n\nDescripción:\n`);
